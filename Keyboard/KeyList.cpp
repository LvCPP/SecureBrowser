...
void AddKey(UINT key_num, UINT key_vk, std::string key_name, std::map <UINT, KeyPair>& key)
{
	if (key_num != key_vk)
	{
		...
		err_file.open("errors.txt", std::ios::ate);
		if (!err_file.is_open())
		{ } //std::cout << "Coudn't open err_file.\n";
		else
		{}//std::cout << "Key number and key vk_code do not coincide!\n";
		return;
	}
	else
	{
		key.emplace(std::make_pair(key_num, KeyPair(key_vk, key_name)));
	}
}
